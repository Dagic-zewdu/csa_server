...
const connectedUsers=async ()=>await userConnection.find({status:'connected'})
/**
 
@param {*} io =>io for broadcisting  
 */
const emitConnections=async (io)=>io.sockets.emit('users',await allConnections()) 
...
const setConnection=async (user,socket)=>{
   ...
   if(findUser.length>0){
      const update=await userConnection.findByIdAndUpdate(findUser[0]._id,user) 
      let connected=await connectedUsers()
        console.log(connected.length+' user connected')
        emitConnections(socket)       
   }
